...

{
    ...
    const AError = SBError.subtype('AError', 'A msg',
            (template, gotDetails) => {
        assert.equal(template, 'A msg', 'Custom rendering method first ' +
                'argument is the template.');
        assert.equal(gotDetails, expectedDetails, 'Custom rendering method ' +
                'second argument is the details of the error.');
        
        return 'rendered!';
    });
    
    const a = new AError(expectedDetails);
    assert.equal(a.message, 'rendered!',
            'Message is as rendered by custom rendering method.');
}

// Subtypes inherit custom renderer.
{
    const expectedDetails = {};
    const AError = SBError.subtype('AError', 'A msg',
            (template, gotDetails) => {
        assert.equal(template, 'B msg', 'Custom rendering method first ' +
                'argument is the template.');
        assert.equal(gotDetails, expectedDetails, 'Custom rendering method ' +
                'second argument is the details of the error.');
    ...
    const b = new BError(expectedDetails);
    assert.equal(b.message, 'rendered!',
            'Message is as rendered by custom rendering method.');
}

// Custom renderer does not effect parent types.
{
    const expectedDetails = {};
    const AError = SBError.subtype('AError', 'A msg');
    const BError = AError.subtype('BError', 'B msg',
    ...
    const a = new AError(expectedDetails);
    assert.equal(
            a.message, 'A msg', 'Message is rendered by default renderer.');
}
